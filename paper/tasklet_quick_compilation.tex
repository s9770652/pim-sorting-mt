...
%	\textcolor{red}{[OneInsertion]}
	...
	\end{itemize}
\end{enumerate}
The results for 32-bit integers are shown in \cref{fig:quick:implementations}.
The numbers are uniformly distributed so the deterministic pivots are of the same quality as the random ones in expectation.
...
\Cref{imp:triviality_before_call}, which does not call \QS{} on trivial partitions, decidedly fares the best out of all handlings, being amongst the top performers across all tested combinations.
