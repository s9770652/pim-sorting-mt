...
The quality of the compilation of \QS{} is highly erratic to such an extent that \Dash even with the same pivots! \Dash one implementation variant may see a reduction of 25\% from another one where none should logically be.
There are small details influencing the runtime.
By way of example, storing the value of the pivot in a dedicated variable instead of accessing it through a pointer changes the runtime by a few percentage points in both directions, depending on the rest of implementation.
But as hinted at in the preceding part of this \lcnamecref{sec:tasklet:quick}, there are four major parameters to examine:
...
Realistically speaking, it should not be needed to check for trivial lengths because even though it is doable with just one additional instruction, such tiny partitions are rare, and \IS{} would terminate after a few instructions anyway.
...
The following \lcnamecrefs{imp:normal} were tested:
...
%	\textcolor{red}{[ThreshThenTriv]}
	\begin{itemize}
		\item
		This \lcnamecref{imp:threshold_then_triviality} is nonsensical from a logical point of view, since if the threshold is not undercut, the length cannot be trivial.
		However, it gives the compiler an explicit guarantee that the loop for partitioning does not end immediately, eliminating a possible cause for the compiler's behaviour.
	\end{itemize}
...
%	\textcolor{red}{[TrivialBC]}
	\begin{itemize}
		\item
		This \lcnamecref{imp:triviality_before_call}, as well as the next two, gets rid of some unneeded uses of \QS{}.
...
Even when ignoring the differences between specific \lcnamecrefs{imp:normal} for now, the high fluctuations between the plots leap to the eye.
...
When focussing on the top-performing \lcnamecrefs{imp:normal}, the increase can reach more than 15\%.
...
\Cref{imp:triviality_before_call}, which does not call \QS{} on trivial partitions, decidedly fares the best out of all \lcnamecrefs{imp:normal}, being amongst the top performers across all tested implementations.
...
Interestingly, for all investigated implementations, the compiler is capable of eliminating the last possible recursive calls by turning them into jumps back to the beginning of their respective function, regardless of whether these were properly tail recursive or not.
...
Even the nonsensical \cref{imp:threshold_then_triviality} manages to take the lead sometimes!
...
Luckily, both use \cref{imp:triviality_before_call} so we decided to make the only difference between the default configurations of 32-bit and 64-bit \QS{} be the usage of recursion (64-bit) and iteration (32-bit).
...
As explained earlier in \enquote{\nameref{sec:tasklet:quick:imp:sentinels}}, the right pointer \lstinline|j| halts ultimately either on the left pointer directly (\lstinline|j == i|) or on the element behind it (\lstinline|j == (i - 1)|).
