...
Finding the right balance between the heightened overhead through multiple \IS{} passes and the shortened runtime of each \IS{} pass is subject to research \cite{skean2023optimization, 10.1007/3-540-44669-9_12} and depends on the cost of the operations (comparing, swapping, looping).
...
When moving to greater input lengths, the differences in performance between the two-round \ShS*{} become more pronounced \cref{fig:shell_three_rounds};
...
On a DPU, this sequence leads to a runtime of near exactly 74.000 cycles, placing it in the middle of the triplets.
...
Two issues come up.
Firstly, greater input lengths require greater steps and those in turn require more sentinel values.
...
\Cref{fig:quick_vs_insert_vs_shell} shows that even though \ShS{} takes just a fraction of the time \IS{} takes \Dash seeming to achieve a runtime between \(\bigomega{n \lb n}\) and \(\bigoh{n \lb^2 n}\) \Dash, \QS{} beats \ShS{} for 28 elements onwards.
This makes \ShS{} the fastest sorting algorithm only for the tiny range from 20 to 27 elements.
