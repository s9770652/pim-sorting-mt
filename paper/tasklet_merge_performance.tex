...
On sorted and almost sorted inputs of length \(n \approx 1000\), it takes the lead over the full-space \MS*{}, since the second runs need not be flushed and the additional copying of the first runs is unrolled.
For other inputs, the half-space \MS{} does not take even 10\% longer than the full-space \MS{} without write-backs for both 32-bit and 64-bit integers.
