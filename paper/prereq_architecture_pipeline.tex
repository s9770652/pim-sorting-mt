...
%	\includegraphics[page=65]{example-image-duck}
	\begin{tikzpicture}[
		...
		% Memories.
		\def\pad{1}
		\draw[fill=black!5] (-\pad-1, -8.5) rectangle +(1, 5) node[stage, midway, rotate=90] {Re\smash{g}ister File};
		\draw[fill=black!5] (\stagewidth+\pad,  -4.5) rectangle +(3, 3) node[stage, midway] {IRAM};
		\draw[fill=black!5] (\stagewidth+\pad, -13.5) rectangle +(3, 5) node[stage, midway] {WRAM};
