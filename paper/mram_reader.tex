...
Such \emph{direct memory accesses} (DMAs) to the MRAM are translated into the assembler instructions \lstinline|ldma| and \lstinline|sdma|, which load and store the particular pieces of data.
...
The size of the sequential-read buffer within the WRAM is determined by the compile-time constant \seqreadcachesize{}, which can be set to either 32, 64, 128, 256, 512, or 1024.
...
All in all, the memory footprint of a sequential-read buffer is at least 2 × \seqreadcachesize{} many bytes but less than 3 × \seqreadcachesize{} many.
...
To load data from the specified MRAM address, the address is rounded down the the next multiple of \seqreadcachesize{}, which yields the beginning of the page containing the MRAM address.
...
To access the current element, one simply dereferences the pointer to the current element.
...
This means that the page which was stored in the second half of the buffer so far is loaded from the MRAM again but stored in the first half this time.
...
Due to the alignment of the buffer on a power of two and due to page sizes being also a power of two, the generation of such a carry bit signifies that the pointer to the current element has left the first buffer half.
