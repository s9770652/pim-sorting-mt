...
A slightly more sophisticated implementation (denoted by \emph{half-space}) needs space for only \(n/2\) additional elements~\cite[Chapter~2.5]{lang2009algorithmen}:
...
During a merger, three pointers are sustained:
two for each run, both pointing to the current element of their respective run, and one pointing to the current output location.
After each comparison and subsequent write, the output pointer and the pointer of the run with the less element are incremented.
The frequency of these incrementations can be reduced by, for example, comparing and writing twice consecutively, where the second time uses offset pointers (similarly to how it was done in case of \IS{}).
Afterwards, the output pointer is incremented by two and either one of the run pointers is incremented by one or both run pointers are incremented by two.
This unrolling turned out to be detrimental to the quality of the compilation and was, hence, dropped.
